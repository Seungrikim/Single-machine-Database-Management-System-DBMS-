...
public class SortOperator extends QueryOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        // TODO(proj3_part1): implement
        Queue<Pair<Record, Integer>> pq = new PriorityQueue<>(runs.size(), new RecordPairComparator());
        Run mergeRun = new Run(this.transaction, getSchema());
        List<Iterator<Record>> iterators = new ArrayList();
        for (int i = 0; i < runs.size(); i ++) {
            iterators.add(i, runs.get(i).iterator());
            if (iterators.get(i).hasNext()) {
                pq.add(new Pair<>(iterators.get(i).next(), i));
            }
        }
        while (!pq.isEmpty()) {
            Pair<Record, Integer> smallRecord = pq.remove();
            mergeRun.add(smallRecord.getFirst());
            if (iterators.get(smallRecord.getSecond()).hasNext()) {
                pq.add(new Pair<>(iterators.get(smallRecord.getSecond()).next(), smallRecord.getSecond()));
            }
        }
        return mergeRun;
    }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        for (int i = 0; i < runs.size(); i += numBuffers - 1) {
            run.add(mergeSortedRuns(runs.subList(i, i + numBuffers - 1)));
        }
        return run;
    }
    ...
    public Run sort() {
        ...
        while (sourceIterator.hasNext()) {
            sourceIterator = getBlockIterator(sourceIterator, getSource().getSchema(), numBuffers);
            sortedRun.add(sortRun(sourceIterator));
        }
        while (sortedRun.size() > 1) {
            sortedRun = mergePass(sortedRun);
        }
        return sortedRun.get(0);
    }
