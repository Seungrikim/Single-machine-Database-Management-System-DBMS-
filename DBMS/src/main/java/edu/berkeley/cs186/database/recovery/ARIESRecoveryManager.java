...
public class ARIESRecoveryManager implements RecoveryManager {
    ...
    public long end(long transNum) {
        ...
        if (transactionEntry.transaction.getStatus().equals(Transaction.Status.ABORTING)) {
            rollbackToLSN(transNum, Long.MIN_VALUE);
            prevLSN = Optional.of(transactionEntry.lastLSN);
        }
        ...
        for (Map.Entry<Long, Long> page : dirtyPageTable.entrySet()) {
            dirtyPageTable.putIfAbsent(page.getKey(), prevLSN.get());
        }
    ...
    private void rollbackToLSN(long transNum, long LSN) {
        ...
        Optional<Long> undo = undohelper(lastRecord, lastRecord.getLSN());
        if (lastRecord.getUndoNextLSN().isPresent() && currentLSN <= LSN) {
            transactionEntry.lastLSN = lastRecordLSN;
            return;
        }
        while (lastRecord.getPrevLSN().isPresent() && lastRecord.getPrevLSN().get() > LSN) {
            lastRecord = logManager.fetchLogRecord(lastRecord.getPrevLSN().get());
            if (currentLSN != 0 && currentLSN <= LSN) {
                break;
            }
            undo = undohelper(lastRecord, lastRecordLSN);
            if (undo.isPresent()) {
                lastRecordLSN = undo.get();
            }
        }
        transactionEntry.lastLSN = lastRecordLSN;
        return;
    ...
    public synchronized void checkpoint() {
        ...
        for (Map.Entry<Long, Long> item : dirtyPageTable.entrySet()) {
            ...
            chkptDPT.put(item.getKey(), item.getValue());
        }
        for (Map.Entry<Long, TransactionTableEntry> item : transactionTable.entrySet()) {
            ...
            Pair<Transaction.Status, Long> pair = new Pair<>(item.getValue().transaction.getStatus(),
                    item.getValue().lastLSN);
            chkptTxnTable.put(item.getKey(), pair);
        }
    ...
    private Optional<Long> undohelper(LogRecord lastRecord, long lastLsn) {
        if (lastRecord.isUndoable()) {
            LogRecord undolsn = lastRecord.undo(lastLsn);
            logManager.appendToLog(undolsn);
            undolsn.redo(this, diskSpaceManager, bufferManager);
            return Optional.of(undolsn.getLSN());
        }
