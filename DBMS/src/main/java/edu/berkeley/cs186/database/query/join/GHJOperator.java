...
public class GHJOperator extends JoinOperator {
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        for (Record build: buildRecords) {
                DataBox buildValue = build.getValue(buildColumnIndex);
                if (!hashTable.containsKey(buildValue)) {
                    hashTable.put(buildValue, new ArrayList<>());
                }
                hashTable.get(buildValue).add(build);
        }

        // Probing stage
        for (Record probe: probeRecords) {
                DataBox probValue = probe.getValue(probeColumnIndex);
                if (!hashTable.containsKey(probValue)) continue;
                // We have to join the right record with each left record with
                // a matching key
                for (Record lRecord : hashTable.get(probValue)) {
                    Record joinedRecord = lRecord.concat(probe);
    ...
    private void run(Iterable<Record> leftRecords, Iterable<Record> rightRecords, int pass) {
        ...
        for (int i = 0; i < leftPartitions.length; i++) {
            ...
            if (leftPartitions[i].getNumPages() <= this.numBuffers - 1 && rightPartitions[i].getNumPages() <= this.numBuffers - 1) {
