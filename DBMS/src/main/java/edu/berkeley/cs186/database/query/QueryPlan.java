...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int costOfseq = minOp.estimateIOCost();
        int exclude = -1;
        List<Integer> eiligibleIndexCol = getEligibleIndexColumns(table);
        for (int i = 0; i < eiligibleIndexCol.size(); i++) {
            SelectPredicate predicate = this.selectPredicates.get(eiligibleIndexCol.get(i));
            QueryOperator isOp = new IndexScanOperator(
                    this.transaction, table,
                    predicate.column,
                    predicate.operator,
                    predicate.value
            );
            int costOfinx = isOp.estimateIOCost();
            if (costOfinx < costOfseq) {
                minOp = isOp;
                exclude = eiligibleIndexCol.get(i);
            }
            costOfseq = minOp.estimateIOCost();
        }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Set<String> prevkey : prevMap.keySet()) {
            for (int i = 0; i < this.joinPredicates.size(); i++) {
                String leftc = this.joinPredicates.get(i).leftColumn;
                String rightc = this.joinPredicates.get(i).rightColumn;
                String leftt = this.joinPredicates.get(i).leftTable;
                String rightt = this.joinPredicates.get(i).rightTable;
                QueryOperator minJoint;
                QueryOperator passOp = null;
                QueryOperator preOp = prevMap.get(prevkey);
                Set<String> newSet = new HashSet<>();
                int minimumcost = Integer.MAX_VALUE;
                if (prevkey.contains(leftt) && !prevkey.contains(rightt)) {
                    for (Set<String> passkey : pass1Map.keySet()) {
                        if (passkey.contains(rightt)) {
                            newSet.addAll(prevkey);
                            newSet.addAll(passkey);
                            passOp = pass1Map.get(passkey);
                        }
                    }
                    minJoint = minCostJoinType(preOp, passOp, leftc, rightc);
                } else if (prevkey.contains(rightt) && !prevkey.contains(leftt)) {
                    for (Set<String> passkey : pass1Map.keySet()) {
                        if (passkey.contains(leftt)) {
                            newSet.addAll(prevkey);
                            newSet.addAll(passkey);
                            passOp = pass1Map.get(passkey);
                        }
                    }
                    minJoint = minCostJoinType(preOp, passOp, rightc, leftc);
                } else {
                    continue;
                }
                if (result.containsKey(newSet)) {
                    minimumcost = result.get(newSet).estimateIOCost();
                }
                if (!result.containsKey(newSet) || minJoint.estimateIOCost() < minimumcost) {
                    result.put(newSet, minJoint);
                }
            }
        }
        return result;
    ...
    public Iterator<Record> execute() {
        ...
        for (int i = 0; i < this.tableNames.size(); i++) {
            ...
            singleSet.add(this.tableNames.get((i)));
            QueryOperator minCost = minCostSingleAccess(this.tableNames.get(i));
            pass1Map.put(singleSet, minCost);
        }
        Map<Set<String>, QueryOperator> minMap = new HashMap<>();
        minMap = pass1Map;
        for (int i = 0; i < this.tableNames.size(); i++) {
            prevMap = minMap;
            minMap = minCostJoins(prevMap, pass1Map);
        }
